    ...
    try {        
        ...
        if(userRole == "student"){
            if(session.user.id === userId){
                return NextResponse.json(
                    { error: "A member is not allowed to modify their own permissions within the organization." },
                    { status: 401 }
                );               
            }
            ...
            else if(!userOrgPermissions?.can_set_member_permissions){
                return NextResponse.json(
                    { error: "Unauthorized" },
                    { status: 401 }
                );                   
        ...
        const result = await db.update(user_org_relations).set({
            ...
            userIdUpdated: user_org_relations.user_id,
