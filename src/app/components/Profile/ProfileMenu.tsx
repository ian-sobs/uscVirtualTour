...
import { studentIcon } from "../../lib/icons";
				...
				<div className="absolute right-0 mt-2 w-56 sm:w-64 bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden z-500">
								...
								<p className="text-sm text-gray-500">
									{session
										? (session.user || session).username ||
										  (session.user || session).displayUsername
										: "Not signed in"}
								</p>
							</div>
						</div>
					...
					<div className="py-1">
						...
						{(session?.user || session)?.is_admin && (
							<Link
								href="/admin"
								className="w-full px-4 py-2 text-left hover:bg-gray-100 text-black flex items-center gap-3 cursor-pointer"
							>
								<svg
									className="w-5 h-5"
									fill="none"
					...
					<div className="border-t border-gray-200 py-1">
						...
						<button
							onClick={() =>
								window.open(
									"https://docs.google.com/forms/d/e/1FAIpQLSfjWuNZGvBVQnA4U5fhCbqhZMjrEnZ5C9oMOua0IjyaQ4fGxw/viewform?usp=dialog",
									"_blank"
								)
							}
